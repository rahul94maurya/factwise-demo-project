...
const celebritySlice = createSlice({
  name: "celebrities",
  initialState: initialCelebrityState,
  reducers: {
    updateCelebrity(state, action) {
      state.celebrities = action.payload;
    },
    deleteCelebrity(state, action) {
      const index = state.celebrities.findIndex(
        (ele) => ele.id === action.payload
      );
      const data = state.celebrities;
      data.splice(index, 1);
      state.celebrities = [...data];
      state.celebrityCopy = [...data];
    },
    updateCelebrityCopy(state, action) {
      state.celebrityCopy = action.payload;
    },
  },
});

export const { updateCelebrity, deleteCelebrity, updateCelebrityCopy } =
